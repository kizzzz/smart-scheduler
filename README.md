...
| 层 | 职责 | 实现 | 出错代价 |
|---|---|---|---|
| L1 意图解析 | 自然语言 → 结构化约束 | GLM（默认 `glm-4.5-flash`，可在界面切换），失败降级为规则解析 | 可控：解析结果全部回显给店长确认 |
...
默认文本模型是 `glm-4.5-flash`：实测 3 个意图 case 全部正确、零编造，代价是一次生成约 50 秒
（意图解析与解释生成各跑一趟 LLM）。这是「准确优先」的显式取舍——意图解析错了，求解器排得再对也是排错了需求。
赶时间可在界面切 `glm-4-flash-250414`（约 6 秒），或部署时设 `GLM_MODEL` 覆盖。
慢模型的超时不用手工调：后端按各模型实测耗时自动放宽（`models_registry.timeout_for`），
否则会出现「选了最准的模型、每次都超时降级成规则解析、结果反而更差」。

**降级是三级而不是一级**。`glm-4.5-flash` 的耗时波动很大（线上实测 43s / 52s / 85s，
也出现过单次打满 75s 预算），只要打满就会掉到规则解析——用户选了「最准」，偶发拿到的却是最差结果。
所以现在是：慢模型超时 → 自动改用最快的 `glm-4-flash-250414` 再试一次 → 仍失败才落到规则解析。
换过模型会在「已理解为」卡片上明确标注「已自动换模型」，不静默替换：用户是照着准确率做选择的。

三处超时预算是一条链，改任何一处都要一起看：
`models_registry.timeout_for`（单次调用 75s）→ 一次生成两趟 LLM（最坏约 150s）→
`deploy/Caddyfile` 的 `read_timeout 180s`。边缘超时小于后端预算，就会出现
「后端还在算、页面先拿到 504」——这个坑已经踩过一次，`test_edge_timeout_covers_worst_case_generate`
会在预算不匹配时直接测试失败。
